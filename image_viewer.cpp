...
private:
  ...
  bool isPng(std::vector<uint8_t> &buff);
  bool parsePng(std::vector<uint8_t> &buff);
...
bool Image::isPng(std::vector<uint8_t> &buff) {
  if (buff.size() < 8) {
    return false;
  }
  if (buff[0] != 0x89) {
...
std::optional<uint32_t> read4BytesAsU32(const std::vector<uint8_t> &buff, int &idx) {
  uint32_t ans = 0;
  if (idx + 4 > buff.size()) {
...
std::optional<std::string> read4BytesAsStr(const std::vector<uint8_t> &buff, int &idx) {
  std::string ans;
  if (idx + 4 > buff.size()) {
    std::cerr << "Unexpected End of file...\n";
    return std::nullopt;
...
bool Image::parsePng(std::vector<uint8_t> &buff) {
  std::cout << "Found Png...\n";
  int idx = 8;
